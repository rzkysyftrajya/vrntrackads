...
export async function GET(request: Request) {
  try {
    ...
    if (!metrics || metrics.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'No campaign metrics found for the last 30 days',
        count: 0,
        data: [],
      });
    }
    ...
    if (error) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Failed to upsert campaign metrics into Supabase',
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully synchronized ${recordsToUpsert.length} campaign metrics records`,
